...
const LoginPage = () => {
    ...
    const submit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                setMissingAccount(data.code === "ACCOUNT_NOT_FOUND");
                return;
            }
    ...
    return (
        <main className="relative min-h-screen overflow-hidden bg-[#05070f] px-5 py-6 text-white">
            ...
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_15%,rgba(217,76,255,0.10),transparent_34%)]" />
                    ...
                    <section className="rounded-lg border border-white/10 bg-[#05070f]/94 p-6 shadow-[0_0_70px_rgba(22,199,255,0.12)] backdrop-blur md:p-8">
                        ...
                        <form onSubmit={submit} className="mt-8 space-y-5">
                            ...
                            <label className="block">
                                ...
                                <input name="password" type="password" required className="auth-input mt-2 w-full rounded-lg border border-white/10 bg-white/[0.06] px-4 py-4 text-white outline-none transition-colors placeholder:text-white/30 focus:border-[#16c7ff] focus:bg-white/[0.09]" />
                            </label>
                            {error && (
                                <div role="alert" className="rounded-lg border border-[#ff5c5c]/40 bg-[#ff5c5c]/16 p-4 text-sm font-bold text-[#ffd0d0]">
                                    <p>{error}</p>
                                    {missingAccount && (
                                        <Link href="/signup" className="mt-3 inline-flex rounded-md bg-white px-3 py-2 text-[#05070f]">
                                            Create account now
                                        </Link>
                                    )}
                                </div>
