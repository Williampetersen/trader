            ...
            <section className="py-16">
                <Container>
                    <div className="grid items-end gap-8 lg:grid-cols-[1fr_0.65fr]">
                        <div>
                            <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-[#d9e2ef] bg-white px-4 py-2 text-sm font-extrabold text-[#304fff] shadow-sm">
                                <FiBookOpen />
                                AI trading authority hub
                            </div>
                            <h1 className="max-w-4xl break-words text-4xl font-extrabold leading-tight text-[#101828] sm:text-5xl md:text-6xl">
                        ...
                        <div className="rounded-[2rem] bg-[#101828] p-6 text-white">
                            <p className="text-sm font-extrabold uppercase tracking-[0.18em] text-primary">Content architecture</p>
                            <div className="mt-5 grid grid-cols-2 gap-4">
                                <Metric value={String(blogArticles.length)} label="SEO articles" />
                                <Metric value={String(blogCategories.length)} label="Topic clusters" />
                                <Metric value={String(blogTags.length)} label="Tags" />
                                <Metric value="20+" label="FAQ blocks" />
                            </div>
                        </div>
                    </div>
                </Container>
                ...
                <Container>
                    <div className="rounded-[2rem] border border-[#d9e2ef] bg-white p-5 md:p-7">
                        <div className="mb-5 flex items-center gap-2">
                            ...
                            <h2 className="text-2xl font-extrabold text-[#101828]">Topic clusters</h2>
                        </div>
                        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                            {blogCategories.map((category) => (
                                <Link key={category.slug} href={`/blog/category/${category.slug}`} className="rounded-2xl border border-[#d9e2ef] bg-[#f8fafc] p-4 transition-colors hover:border-[#304fff] hover:bg-white">
                                    <h3 className="font-extrabold text-[#101828]">{category.name}</h3>
                                    <p className="mt-2 text-sm text-[#667085]">{category.description}</p>
                                    <span className="mt-4 inline-flex items-center gap-2 text-sm font-extrabold text-[#304fff]">Explore <FiArrowRight /></span>
                                </Link>
                            ))}
                        </div>
                    </div>
            ...
            <section className="py-10">
                <Container>
                    ...
                    <div className="mt-8 rounded-[2rem] border border-[#d9e2ef] bg-white p-5">
                        <h2 className="text-2xl font-extrabold text-[#101828]">Popular tags</h2>
