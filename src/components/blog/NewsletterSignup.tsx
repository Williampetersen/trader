...
const NewsletterSignup = ({ source = "blog" }: { source?: string }) => {
    ...
    return (
        <form onSubmit={submit} className="rounded-[2rem] border border-[#d9e2ef] bg-white p-5 shadow-sm">
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-[#304fff]">Free guide</p>
            <h3 className="mt-2 text-2xl font-semibold text-[#101828]">Get the AI Trading Checklist</h3>
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-[1fr_1.2fr_auto]">
                ...
                <button disabled={status === "saving"} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[#304fff] px-5 py-3 font-semibold text-white transition-colors hover:bg-[#243cc7] disabled:opacity-60">
