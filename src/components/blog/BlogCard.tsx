    ...
    <article className="group overflow-hidden rounded-[2rem] border border-[#d9e2ef] bg-white shadow-sm transition-transform hover:-translate-y-1">
        <Link href={`/blog/${article.slug}`} className="block">
            <div className="relative h-48 overflow-hidden bg-[#101828]">
                ...
                <div className="absolute left-4 top-4 rounded-full bg-primary px-3 py-1 text-xs font-semibold text-black">{article.category.name}</div>
            </div>
            <div className="p-6">
                <p className="text-sm font-bold text-[#667085]">{article.readingTime} | {article.updatedAt}</p>
                <h2 className="mt-3 text-2xl font-semibold leading-tight text-[#101828] group-hover:text-[#304fff]">{article.title}</h2>
                <p className="mt-3 line-clamp-3 text-[#667085]">{article.excerpt}</p>
                <span className="mt-5 inline-flex items-center gap-2 font-semibold text-[#304fff]">
                    Read article
                    <FiArrowRight />
                </span>
            </div>
